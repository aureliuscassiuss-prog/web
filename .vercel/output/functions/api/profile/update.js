...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (semester)
            updateFields.semester = parseInt(semester);
        if (college)
        ...
        // Update user
        const result = await db.collection('users').findOneAndUpdate({ _id: new ObjectId(decoded.userId) }, { $set: updateFields }, { returnDocument: 'after' });
        if (!result) {
