                ...
                <div className="flex-1 mr-2 flex flex-wrap gap-2">
                    ...
                    {resource.status && (
                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${
                            resource.status === 'approved' ? 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800' :
...
export default function ResourceGrid({ view, filters, searchQuery = '', onUploadRequest }: ResourceGridProps) {
    ...
    const [chatMessages, setChatMessages] = useState<Message[]>([
        ...
        ng, setIsAiLoading] = useState(false)
        
        
        ration States
        g, setGenerating] = useState(false)
        eft, setAttemptsLeft] = useState(3)
        ...
        const params = new URLSearchParams()
            ...
            // --- AI HANDLERS ---
        t handleSendMessage = async () => {
            !chatInput.trim() || isAiLoading) return
            t userMessage = chatInput.trim()
                setChatInput('')
            sAiLoading(true)

            t newMessages: Message[] = [...chatMessages, { role: 'user', content: userMessage }]
                essages(newMessages)
        ...
        t clearChat = () => {
            hatMessages([{ role: 'assistant', content: "Chat cleared. How can I help you now?" }])
        ...
        if (activeTab !== 'ai') fetchData()
                ...
                tiveTab, searchQuery, filters, token])
                
                Generation Functions
                ormDataForPdf = (aiData: any, filters: any) => {
            t flat: any = {
            ...
            t sectionA = aiData.sections?.find((s: any) => s.name.includes('Section A'))
            sectionA && sectionA.questions) {
            ...
            t sectionB = aiData.sections?.find((s: any) => s.name.includes('Section B'))
        ...
        doc.setTextColor(255, 0, 0);
            ...
            t splitDisclaimer = doc.splitTextToSize(disclaimer, 130);
            text(splitDisclaimer, 10, 54);
            setTextColor(0, 0, 0);

            yPos = 54 + (3 * splitDisclaimer.length) + 5;
            ...
            const optB = `(b) ${data[`${key}_OPTION_B`] || ''}`;
                t optC = `(c) ${data[`${key}_OPTION_C`] || ''}`;
                t optD = `(d) ${data[`${key}_OPTION_D`] || ''}`;
            ...
            yCol2 += hRow2 + 3;
                ...
                // Render Q10
            t q10Key = "MCQ_10";
            t q10Text = data[`${q10Key}_QUESTION`];
            q10Text) {
        ...
        const q2Parts = [
            ...
            if (data[`Q${q}_OR_PART`]) {
                ...
                .Q6_PART_1) {
                    ont("times", "normal");
                    ("Q.6", 150, yCol2);
                text("Attempt any two:", 156, yCol2);
                    yCol2 += 5;
        ...
        const filename = `Sample_Paper_${data.SUBJECT_NAME.replace(/\s+/g, '_')}_${new Date().getTime()}.pdf`
            save(filename)
                ...
                GeneratePaper = async () => {
                    le !== 'admin' && attemptsLeft <= 0) {
                    ou have reached your daily limit. Try again tomorrow!')
            ...
            ndleDelete = async (resourceId: string) => {
        if (!confirm('Are you sure you want to delete this resource?')) return
            ...
            AIN RENDER ---
                g) {
                    ...
                    ssName="flex h-64 flex-col items-center justify-center space-y-3">
                     className="h-8 w-8 animate-spin rounded-full border-2 border-gray-200 border-t-blue-600 dark:border-gray-800 dark:border-t-blue-400"></div>
                    lassName="text-sm text-gray-500 animate-pulse">Loading...</p>
                v>
            
            }
            
             === 'uploads') return <UploadsView uploads={uploads} onUploadRequest={onUploadRequest} onDelete={handleDelete} />
                 'leaderboard') return <LeaderboardView leaderboard={leaderboard} />
                    
                    
                        animate-fade-in w-full max-w-4xl mx-auto">
                            
                        YOUT TABS */}
                    <div className="mb-6 bg-gray-100/50 dark:bg-gray-800/50 p-1.5 rounded-xl border border-gray-200 dark:border-gray-800">
                        ssName="grid grid-cols-4 gap-1">
                            on active={activeTab === 'notes'} onClick={() => setActiveTab('notes')} label="Notes" icon={<FileText className="w-4 h-4" />} />
                                ctive={activeTab === 'pyqs'} onClick={() => setActiveTab('pyqs')} label="PYQs" icon={<FileQuestion className="w-4 h-4" />} />
                                    e={activeTab === 'formula'} onClick={() => setActiveTab('formula')} label="Formula" icon={<FileText className="w-4 h-4" />} />
                                    e={activeTab === 'ai'} onClick={() => setActiveTab('ai')} label="AI Tutor" icon={<Sparkles className="w-4 h-4" />} isSpecial />
                                        ...
                                        4">
                                            WITH UPLOAD BUTTON */}
                                         items-center justify-between pb-2 border-b border-gray-100 dark:border-gray-800">
                                    e="text-base sm:text-lg font-bold text-gray-900 whitespace-nowrap dark:text-white">
                                    ab === 'notes' ? 'Lecture Notes' : activeTab === 'pyqs' ? 'Previous Papers' : 'Formula Sheets'}
                            ...
                            {onUploadRequest && (
                                ...
                                >
                                    ...
                                    lt Counter */}
                                n className="text-[10px] sm:text-xs font-medium px-2 py-1 bg-gray-100 whitespace-nowrap dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full">
                    ...
                    filters={filters}
                    activeTab={activeTab}
                        neratePaper={handleGeneratePaper}
                            ting={generating}
                                
                            
                            rid Layout: 1 column mobile, 2 columns desktop
                                ssName="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ources.map((resource) => (
                        ...
                        /* AI TUTOR INTERFACE */
                     className="flex flex-col h-[600px] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden bg-gray-50 dark:bg-gray-900/50 relative shadow-sm">
                    {/* Chat Header */}
                         className="absolute top-0 inset-x-0 p-3 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 flex justify-between items-center z-10">
                             className="flex items-center gap-3">
                                 className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center shadow-indigo-500/20">
                                     className="w-5 h-5 text-white" />
                                    
                                    <div>
                                        sName="text-sm font-bold text-gray-900 dark:text-white leading-none">AI Tutor</h3>
                                            ="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">Always here to help</p>
                                                
                                            
                                                at} className="p-2 hover:bg-red-50 hover:text-red-500 rounded-lg text-gray-400 transition-colors" title="Clear Chat">
                                            e="w-4 h-4" />
                            ...
                            <div className="flex-1 overflow-y-auto p-4 pt-16 pb-4 space-y-6 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-800">
                                    s.map((message, index) => (
                                    ={index} className={`flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
                                        <div className={`flex gap-2 max-w-[90%] md:max-w-[80%] ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                            ar */}
                                            ssName={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-1 overflow-hidden
                                        e.role === 'user' ? 'bg-blue-100 text-blue-600' : 'bg-violet-100 text-violet-600'}`}>
                                        {message.role === 'user' ? (
                                            user?.avatar ? (
                                            ...
                                            le */}
                                            ssName={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed shadow-sm
                                        e.role === 'user'
                                ...
                                ding && (
                                 className="flex gap-2 justify-start ml-1">
                                ...
                                ea */}
                            ssName="p-3 bg-white dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800">
