...
export default function ProfileBanner({ user, children }: ProfileBannerProps) {
    ...
    return (
        <div className={`relative overflow-hidden rounded-t-xl bg-gradient-to-br ${getHeroBackground()} backdrop-blur-xl border-b border-white/40 dark:border-gray-700/30 shadow-sm transition-colors duration-1000 min-h-[180px] p-5 sm:p-6`}>
            ...
            <div className="absolute top-0 right-0 w-full h-full pointer-events-none overflow-hidden">
                {timeOfDay === 'morning' && (
                    <>
                        ...
                        <Sun className="absolute top-8 right-8 w-24 h-24 text-orange-400/20 rotate-12" />
                        <CloudSun className="absolute top-16 right-24 w-16 h-16 text-white/40 dark:text-white/10" />
                    </>
                )}
                {timeOfDay === 'afternoon' && (
                    <>
                        <div className="absolute -top-20 -right-20 w-80 h-80 bg-yellow-200/50 rounded-full blur-[80px]" />
                        <Sun className="absolute -top-6 -right-6 w-32 h-32 text-yellow-500/20 animate-spin-slow" style={{ animationDuration: '20s' }} />
                    </>
                )}
                {timeOfDay === 'evening' && (
                    <>
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div className="flex flex-col gap-0.5 mb-10 sm:mb-0 sm:ml-auto sm:text-right max-w-md">
                    {/* 
                      Strategy: On mobile, add bottom margin (mb-10) to clear the avatar.
                      On desktop (sm), move text to the right (sm:ml-auto sm:text-right) so the left is free for the avatar.
                   */}
                    <p className="text-xs sm:text-sm font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider mb-0.5">
                        {greeting}
                    </p>
                    <h1 className="text-2xl sm:text-4xl font-black text-gray-900 dark:text-white tracking-tight leading-none">
                        {user?.name || 'Scholar'}
                    </h1>
                    <p className="text-xs sm:text-sm text-gray-700 dark:text-gray-200 mt-1 font-medium opacity-90">
