    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {subjects.map(subject => {
                ...
                return (
                    <div key={subject.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 shadow-sm hover:shadow-md transition-all duration-300 group">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h3 className="font-bold text-lg text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{subject.name}</h3>
                                <p className="text-xs text-gray-500 dark:text-gray-400 font-mono mt-1">{subject.code}</p>
                            </div>
                            <div className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-bold ${isLow ? 'bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400' : 'bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400'}`}>
                                {percentage}%
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <div className="flex justify-between text-xs mb-1.5">
                                    ...
                                    <span className="font-medium text-gray-900 dark:text-white">{subject.attendedClasses}/{subject.totalClasses} Classes</span>
                                </div>
