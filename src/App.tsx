...
function Layout({
  ...
  return (
    <div className={`flex flex-col bg-white dark:bg-black text-gray-950 dark:text-gray-50 transition-colors duration-200 font-sans selection:bg-gray-900 selection:text-white dark:selection:bg-gray-100 dark:selection:text-black ${(isChatPage || isStanadalonePage) ? 'h-[100dvh] overflow-hidden' : 'min-h-screen'}`}>
      ...
      {!['/', '/resources', '/preparation', '/ai-assistant', '/ai-papers', '/attendance', '/cgpa-calculator', '/pomodoro', '/pdf-generator', '/medical-generator', '/chat', '/video-chat', '/coffessions', '/events', '/omex', '/leaderboard', '/profile', '/uploads', '/saved-resources'].some(path => location.pathname === path || location.pathname.startsWith(path + '/')) && <Footer />}
